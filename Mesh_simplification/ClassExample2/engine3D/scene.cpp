...
Scene::Scene(glm::vec3 position,float angle,float hwRelation,float near, float far)
{
	...
	void Scene::addShape(const std::string& fileName,bool d,int maxfaces)
	{
		Shape* newShape= new Shape(fileName, d, maxfaces);
		glm::vec3 pos = glm::vec3(3.0f, 0.0f, 0.0f);
		if (d)
		{
			newShape->myTranslate(pos,1);
			newShape->changeCenterOfRotation(pos);
		}
		else
		{
	...
	void Scene::addShape(const std::string& fileName,const std::string& textureFileName,bool d,int max_faces)
	{
		shapes.push_back(new Shape(fileName,textureFileName,d,max_faces));
	}
	...
	void Scene::draw(int shaderIndx,int cameraIndx,bool drawAxis)
	{
		...
		{
			shapes[i]->myRotate(0.03, glm::vec3(0, 1, 0));
